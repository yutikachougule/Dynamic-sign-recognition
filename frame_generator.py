...
class VideoFrameGenerator(keras.utils.Sequence):
    ...
    def sampling_augmentation(self, sequence):
        ...
        """
        # Can add other transformations as well, as listed above

        transformations = ['theta','flip_horizontal','brightness']

        # random choice of number of transformations
        random_transforms = np.random.randint(2, 4) 
        ...
        for idx in transforms_idxs:
            if transformations[idx] == 'theta':
                transform_parameters['theta'] = np.random.randint(-5, 5)

            elif transformations[idx] == 'tx':
                transform_parameters['tx'] = np.random.randint(-10, 10)

            elif transformations[idx] == 'ty':
                transform_parameters['ty'] = np.random.randint(-15, 15)

            elif transformations[idx] == 'zx':
                transform_parameters['zx'] = np.random.uniform(0.6, 1.05)

            elif transformations[idx] == 'zy':
                transform_parameters['zy'] = np.random.uniform(0.6, 1.05)

            elif transformations[idx] == 'flip_horizontal':
                transform_parameters['flip_horizontal'] = True

            elif transformations[idx] == 'brightness':
                transform_parameters['brightness'] = np.random.uniform(0.4, 0.6)
